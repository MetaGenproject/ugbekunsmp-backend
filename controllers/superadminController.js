...
export const getPlatformStats = async (req, res) => {
    try {
        ...
        const platformRevenue = schools.reduce((acc, school) => acc + (school.revenue || 0), 0);

        const thirtyDaysAgo = new Date();
        thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
        const newSignups = await School.countDocuments({ createdAt: { $gte: thirtyDaysAgo } });

        res.status(200).json({
            success: true,
            data: {
                totalSchools,
                activeUsers: activeUsersCount + activeStudentsCount,
                platformRevenue,
                newSignups
            }
        });
